...
{
    public static RE_NetWorkManager_HL instance;
    ...
    {
        instance = this;
        slotCount = PhotonNetwork.CurrentRoom.PlayerCount;
    ...
    public void SelectDummy()
    {
        ...
        if (PlayerController.instance.setActive == true)
        {
            ...
            slotinPlayercount = PlayerController.instance.selectPlayerCount;
            // print(slotPlayer + "-----여기다!-----");
            GameObject dummySlot = dummyCharacter[slotinPlayercount/*slotCount-1*/];
            //dummyPlayer = Instantiate(dummySlot, PlayerController.instance.slotPlayer[slotCount - 1].transform);
            dummyPlayer = PhotonNetwork.Instantiate(dummySlot.name, PlayerController.instance.slotPlayer[slotCount - 1].transform.position, Quaternion.identity);
